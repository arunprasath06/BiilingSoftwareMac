        ...
        if os.path.exists('data/browser.cfg'):
            with open('data/browser.cfg', 'r') as f:
                ...
                if os.path.exists(location):
                    ...
                    notification_msg('Chrome Location Incorrect',
                                     'Please setup browser location in \n"data/browser.cfg"')
                    return False
        else:
...
class ToPDF:
    def __init__(self, invoice):
        ...
        if chrome_path:
            html_file_path = "//data/outputs/invoice.html"  # Replace with the actual file path of your HTML file
...
class GenerateReceipt:
    ...
    def __init__(self, receipt_id):
        ...
        error = False
        try:
            date, invoice_id, method_id, amount = conn.execute(
                f"SELECT date,invoice,method,amount from payment WHERE id='{receipt_id}'").fetchone()
            ...
            name, address, tel, email = conn.execute(
                f"SELECT name,address,tel,email FROM customer WHERE id={customer_id}").fetchone()
            total = conn.execute(f"SELECT SUM(price) FROM service WHERE invoice={invoice_id}").fetchone()[0]
            paid = \
            conn.execute(f"SELECT SUM(amount) FROM payment WHERE invoice={invoice_id} and id<={receipt_id}").fetchone()[
                0]
...
class GeneratePaymentSummary:
    ...
    def __init__(self, invoice_id):
        ...
        error = False
        try:
            total = conn.execute(f"SELECT COALESCE(SUM(price),0) from service WHERE invoice='{invoice_id}'").fetchone()[
                0]
            customer_id = conn.execute(f"SELECT customerid from invoice WHERE id={invoice_id}").fetchone()[0]
            name, address, tel, email = conn.execute(
                f"SELECT name,address,tel,email from customer WHERE id={customer_id}").fetchone()
            ...
            payment_list = conn.execute(
                f"SELECT id,date,method,amount FROM payment WHERE invoice={invoice_id}").fetchall()
